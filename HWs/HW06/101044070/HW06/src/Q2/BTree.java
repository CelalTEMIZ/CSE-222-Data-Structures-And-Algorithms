...
public class BTree<E extends Comparable<E>> implements SearchTree<E> {
    ...
    /**
     * BinarySearch method of the BTree
     * @param item  The item to insert
     * @param data  The data array
     * @param left  The start index of array
     * @param right The size of array
     * @return The position of added item in the data array
     */
    public int binarySearch(E item, E [] data, int left, int right) {

        if(data[left]!=null && left<order)
        {
            return left+1;
        }
        ...
        while (left<=right) {
            ...
            // Look at the midle index of data array
            int mid=(left + right)/2;

            // if the item is in the middle of the data array, index will be in the middle position
            if (data[mid].compareTo(item)==0) {
                index=mid;
            }
            // if the item is bigger than the middle of the data array, look at the right side
            else if (data[mid].compareTo(item)<0) {
                left=mid + 1;
                index=left;
            } else {
                // Otherwise, look at the left ride of the data array
                right=mid - 1;
                index=right;
            }
        }
